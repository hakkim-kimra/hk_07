...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input { width: 100%; background: #0f172a; border: 1px solid #334155; padding: 15px; border-radius: 12px; color: #fff; font-size: 16px; outline: none; transition: 0.3s; }
        input:focus { border-color: #06b6d4; box-shadow: 0 0 0 3px rgba(6, 182, 212, 0.1); }
        .btn-sim { width: 100%; background: linear-gradient(90deg, #06b6d4, #3b82f6); color: #fff; padding: 15px; border: none; border-radius: 12px; font-weight: 700; font-size: 16px; cursor: pointer; transition: 0.2s; }
        ...
        <div class="sim-card">
            ...
            <form method="POST" style="width: 100%;">
                <div class="input-group">
                    ...
                    <input type="text" name="item" placeholder="e.g. New Headphones" required value="<?= isset($_POST['item']) ? htmlspecialchars($_POST['item']) : '' ?>">
                </div>
                <div class="input-group">
            ...
            <?php if($sim_result): ?>
                <div class="result-box" style="width: 100%;">
                    ...
                    <div style="font-size: 10px; color: #94a3b8; margin-top: 5px; text-align: right;">Risk Level</div>

                    <div class="comparison-grid">
                        <div class="comp-item"><h4>Current Balance</h4><div style="color: #94a3b8;">₹<?= number_format($current_balance) ?></div></div>
                        <div class="comp-item"><h4>After Purchase</h4><div style="color: <?= $color ?>;">₹<?= number_format($future_balance) ?></div></div>
                        <div class="comp-item"><h4>Current Daily Limit</h4><div style="color: #94a3b8;">₹<?= number_format($current_daily_limit) ?></div></div>
                        <div class="comp-item"><h4>New Daily Limit</h4><div style="color: <?= $color ?>;">₹<?= number_format($new_daily_limit) ?></div></div>
                    </div>
                </div>
            <?php endif; ?>
        </div>
